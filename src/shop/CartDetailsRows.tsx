...
const CartDetailsRow = ({item, handleChange, removeFromCart}: any) => {
...
export const CartDetailsRows = (props: any) => {
  ...
  const handleChange = (product: IProduct, event: any) => {
  ...
  if (!props.cart || props.cart.length === 0) {
    ...
    return <React.Fragment>
      { props.cart.map((item: any) => <CartDetailsRow item={item} handleChange={handleChange} removeFromCart={props.removeFromCart} />) }
      <tr>
